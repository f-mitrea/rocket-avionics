...
csv_path = sys.argv[1]
out_path = (
    pathlib.Path(sys.argv[2]) if len(sys.argv) > 2 else ROOT / "figures" / "replay.png"
)
...
ax.plot(
    t[:end], raw[:end], color="#9aa0a6", linewidth=1, label="raw pressure -> altitude"
)
ax.plot(
    t[:end],
    filt[:end],
    color="#1f6feb",
    linewidth=2,
    label="filtered (median 25 + average 3)",
)

marks = [
    (apogee_i, raw, "highest raw sample", "#202124", (12, -34)),
...
ax.set_title(
    "Recorded flight replayed through flight_state.py", fontsize=10, loc="left"
)
fig.tight_layout()
